...
project_SRE = 2261
hobel_path = "C:/Users/mija.frossard/Documents/OneDrive - HESSO/RaD/Projet_Reuse-LCA/Cas études/Hobelwerk/ZH_Hobelwerk_Reuse_LCA_Calculation_vFP.xlsx"

# Code
hobel_table = pd.read_excel(hobel_path, sheet_name="LCI+LCIA", header=2)
hobel_param = pd.read_excel(hobel_path, sheet_name="Parameters", header=1, nrows=1)
hobel_res_config = pd.read_excel(hobel_path, sheet_name="Parameters", header=9, nrows=1)
results_factor = hobel_res_config.iloc[0]["Results factor"]

new_inventory = hobel_table[hobel_table["Status"]=="New"]
reuse_inventory = hobel_table[hobel_table["Status"]=="Reused"]

project_SRE = hobel_param["Building SRE (m2)"]

gwp_reuse_results = ["GWP_A1-A3", "GWP_A4", "GWP_B4", "GWP_C1-C4", "GWP_Total"]
...
for r, c in zip([x for x in gwp_reuse_results if x not in ["GWP_A5", "GWP_B4", "GWP_Total"]], colors):
    step = r.replace("GWP_","")
